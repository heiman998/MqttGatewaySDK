...
/*
 * User requested use of relative addressing for meta-data.
 */
A_UINT32 use_relative_addrs = 0; /* default to absolute addresses */

/*
 * In memory-based DataSet images, meta-data is always 32-bit word-aligned
 * so the low bit is 0.  We use that low bit as a flag to indicate "relative
 * addressing" to reach the next meta-data.  See also "_mem_dset_address"
 * in ROM.
 */
#define DSET_ADDR_FLAG_RELATIVE 0x1
...
add_dset_to_image(A_UINT16 dsetid, int dset_flags, char *dset_filename)
{
    ...
    if ((dset_fd = open(dset_filename, O_RDONLY)) < 0) {
        WARNING("Cannot open DataSet: %s.  IGNORING!\n", dset_filename);
        return;
    }
...
parse_dset_desc_file(void)
{
     ...
     */
    {
        ...
        for (i=0; i<newmap.map_count; i++) {
            ...
            if (i == newmap.map_count-1) {
                /* Last entry in RAM -- point to first entry in ROM */
                newmap.map_entry[i].desc.next =
                    (struct dset_descriptor_s *)oldmap.map_entry[0].targaddr;
            } else {
                if (use_relative_addrs) {
                    newmap.map_entry[i].desc.next =
                        (struct dset_descriptor_s *)(DSET_ADDR_FLAG_RELATIVE | sizeof(dset_descriptor_t));
                } else {
                    newmap.map_entry[i].desc.next =
     ...
     */
    {
        ...
        for (i=0; i<newmap.map_count; i++) {
            if (newmap.map_entry[i].desc.data_type == DSET_TYPE_BPATCHED) {
                ...
                if (use_relative_addrs) {
                    tmp = (void *)(DSET_ADDR_FLAG_RELATIVE |
                        (((char *)newmap.map_entry[i].desc.DataPtr) - (dsetimg_start+sizeof(dset_descriptor_t)*i)));
                } else {
                    /* Adjust DataPtr to absolute Target Memory */
                    tmp = (void *)TO_MEM(newmap.map_entry[i].desc.DataPtr);
                }

                newmap.map_entry[i].desc.DataPtr = tmp;
            }
        }
    }
...
write_map_to_file(void)
{
    ...
    fprintf(newmap_file, "#%s Load address: 0x%x\n",
        use_relative_addrs ? " Relocatable" : "",
...
scan_args(int argc, char **argv)
{
    ...
    while (1) {
        ...
        c = getopt_long (argc, argv, "d:o:I:O:e:s:p:rv",
	...
	case 'r':
            /* Make a RAM DataSet image "relocatable": Use relative addressing */
	    use_relative_addrs = 1;
	    break;
...
main(int argc, char *argv[], char *arge[])
{
    ...
    printf("%sDataSet load address: 0x%x\n",
        use_relative_addrs ? "Relocatable " : "",
        TO_MEM(dsetimg_start));
