					...
					if(jsEnd == NULL)
					{
						...
						while(json_child != NULL)
						{
							if(strcmp_ext(json_child->string, "id") == 0)
							{
								...
								subOtaRam.subFileVersion = (((json_child->valueint + DEV_SOFT_VERSION_BASIC_VALUE) /10) << 4) + (json_child->valueint % 10);//云智易版本号加上basic value就等于实际版本号
...
int16_t userUpdateGwNameToCloudeForHttps(int sslFd)
{
	...
	sprintf(devInfo,"{\"name\":\"%s\"}\r\n\r\n",hmIotsDevInfo.gwName);
        ...
        strcat_ext(sendBuf,"\r\n\r\n");//由于该sdk的sprintf只能操作长度小于256字节的字符串，所以结束符放到了第二段字符串上
        strcat_ext(sendBuf,devInfo);
        printf("fresh:len%d,data:%s\r\n",strlen(sendBuf),sendBuf);
        res = userHttpsClientSendAndRcvResponse(sslFd,sendBuf,rcvBuf);
        do{
	...
	while(1)
	{
		switch(httpState)
		{
			...
			case HTTPS_STATE_CONNECTING:
				...
				qcom_tcp_conn_timeout(5);//防止connect 阻塞
				...
				if (res != A_ERROR)
				{
					...
					if(connectNum > 10)
					{
						...
						qca_ssl_close(&fd);//防止在链接次数到了后，error中的重复close
					}
				}
				break;
			...
			case HTTPS_STATE_FIRST_FLASH_TOKEN:
				if(0)//(userWifiDevRefreshTokenForHttps(fd) < 0)
				{
					...
					if(otaStationInfo.needDownloadImageFromCloud == 1)//首先判断是否需要从云端下载zc固件
					{//ZC固件无效时才会做次操作，以防固件损坏后无法正常运行
			...
			case HTTPS_STATE_SCAN_ZC_NEW_SOFTVERSION:
				...
				if(res >= 0)
				{
					//获取到最新sub firmware信息后，比对是否需要下载该固件
					//固件下载在wifiota C文件中
			...
			case HTTPS_STATE_UPDATE_DEV_INFO_TO_CLOUD_FOR_ECHO:
				...
				{
					echoUpdateNum ++;
					if(echoUpdateNum >= 3)//三次上传失败后，退出上传
					{
						echoUpdateNum = 0;
						httpState = HTTPS_STATE_IDLE;
					}else{
				...
				break;
			/*case HTTPS_STATE_UPDATE_GW_NAME_TO_CLOUD://上电第一次链接网络执行一次，后续不会执行
				printf("update gw name to cloud\r\n");//不是设备端的接口，调用此接口一直提示access-token错误
